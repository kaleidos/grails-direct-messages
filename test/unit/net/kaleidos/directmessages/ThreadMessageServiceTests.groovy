...
class ThreadMessageServiceTests {
    ...
    void testNotShowDeleteMessageOnGetThreads() {
        ...
        assert threadMessageService.getThreads(3, true).size() == 2

        assert threadMessageService.getThreads(1, false).size() == 5
        assert threadMessageService.getThreads(2, false).size() == 3
        assert threadMessageService.getThreads(3, false).size() == 2
        ...
        assert threadMessageService.getThreads(1, true).size() == 4
        assert threadMessageService.getThreads(2, true).size() == 3

        assert threadMessageService.getThreads(1, false).size() == 4
        assert threadMessageService.getThreads(2, false).size() == 3

        //Delete same thread for the other user
        threadMessageService.deleteMessagesOnThread(2, message1)
        assert threadMessageService.getThreads(1, true).size() == 4
        assert threadMessageService.getThreads(2, true).size() == 2

        assert threadMessageService.getThreads(1, false).size() == 4
        assert threadMessageService.getThreads(2, false).size() == 2
